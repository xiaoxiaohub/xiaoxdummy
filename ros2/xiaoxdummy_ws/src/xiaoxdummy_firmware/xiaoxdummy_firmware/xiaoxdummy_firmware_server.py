...
from xiaoxdummy_interface.srv import InitFirmware, ReadFirmware, WriteFirmware
...
class XiaoxdummyFirmwareServer(Node):
    def __init__(self):
        super().__init__('xiaoxdummy_firmware_server')
        ...
        self.create_service(InitFirmware, 'init_firmware', self.init_firmware_callback)
        self.create_service(WriteFirmware, 'write_firmware', self.write_firmware_callback)
        self.create_service(ReadFirmware, 'read_firmware', self.read_firmware_callback)

        self.get_logger().info(
            f'Ready to bridge Xiaoxdummy firmware on {self.serial_port} @ {self.baud_rate} baud'
        )

    def init_firmware_callback(self, request, response):
        with self.lock:
            ...
            if success:
                self.get_logger().info(f'Firmware action {action}: {message}')
            else:
                self.get_logger().error(f'Firmware action {action} failed: {message}')
            return response

    def write_firmware_callback(self, request, response):
        with self.lock:
            response.success = False
            response.message = ''

            if len(request.pos_commands) != NUM_JOINTS:
                response.message = f'expected {NUM_JOINTS} joint commands, got {len(request.pos_commands)}'
                return response
            ...
            command = '>{}\n'.format(
                ','.join(f'{float(pos):.2f}' for pos in request.pos_commands) +
                f',{self.command_speed:.2f}'
            )
    ...
    def read_firmware_callback(self, request, response):
        with self.lock:
            response.success = False
            response.message = ''
            response.pos_commands = list(self.joint_positions)
            response.vel_commands = list(self.joint_velocities)
